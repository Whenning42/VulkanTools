...
class CommandTreeWidgetItem : QTreeWidgetItem {
    ...
    CommandTreeWidgetItem(){};

    CommandTreeWidgetItem(const std::string& name) { setText(0, QString::fromStdString(name)); }
   ...
   public:
    ...
    static CommandTreeWidgetItem* NewCommandWidget(const std::string& name, const CommandRef& command_location) {
        CommandTreeWidgetItem* widget = new CommandTreeWidgetItem(name);
        widget->SetCommandLocation(command_location);
    }
    ...
    bool IsCommand() { return RoleIsSet(kCommandLocationRole); }
